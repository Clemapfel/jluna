...
namespace jluna
{
    ...
    {
        jl_assert_type((unsafe::DataType*) jl_typeof((unsafe::Value*) value), jl_symbol_type);
    }

    Symbol::Symbol(Proxy* owner)
        : Proxy(*owner)
    {
        jl_assert_type((unsafe::DataType*) jl_typeof((unsafe::Value*) owner->operator jl_value_t *()), jl_symbol_type);
    }
